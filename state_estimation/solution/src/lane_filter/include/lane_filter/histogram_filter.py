...
def histogram_predict(belief, dt, left_encoder_ticks, right_encoder_ticks, grid_spec, robot_spec, cov_mask):
        ...
        # v = 1.0 # replace this with a function that uses the encoder 
        # w = 0.0 # replace this with a function that uses the encoder
        d_l = 2*math.pi*robot_spec["wheel_radius"]*left_encoder_ticks/robot_spec["encoder_resolution"]
        d_r = 2*math.pi*robot_spec["wheel_radius"]*right_encoder_ticks/robot_spec["encoder_resolution"]
        dist = (d_l + d_r)/2
        delta_phi = (d_r-d_l)/robot_spec["wheel_baseline"]
        
        # TODO propagate each centroid forward using the kinematic function
        d_t = grid_spec['d'] + dist*np.sin(delta_phi)# replace this with something that adds the new odometry
...
def generate_measurement_likelihood(segments, road_spec, grid_spec):
    ...
    for segment in segments:
        ...
        i = int(floor((d_i - grid_spec['d_min'])/grid_spec['delta_d'])) # replace with something that accounts for the movement of the robot
...
def histogram_update(belief, segments, road_spec, grid_spec):
    ...
    if measurement_likelihood is not None:
        ...
        belief = belief/np.sum(belief) # replace this with something that combines the belief and the measurement_likelihood
